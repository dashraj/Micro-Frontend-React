...
const indexCss = import.meta.glob('/src/index.css', { as: 'raw', eager: true });
...
const AppPlusShadowStyles = (props: AppExternalProps) => {
  ...
  useEffect(() => {
    if (topRef && topRef.current && topRef.current.parentNode) {
      ...
      (topRef.current.parentNode as ShadowRoot).adoptedStyleSheets = [...styleSheets];
    }
  }, []);
  ...
  return (
    <section id='widget' ref={topRef} className='h-100'>
      {shadowRoot && <App target={shadowRoot as unknown as Element} {...props} />}
    </section>
  );
};

const APP_EXTERNAL_PROPS: Partial<Record<keyof AppExternalProps, "string" | "number" | "boolean" | "function" | "json">> = {
  customerId: "string",
  category: "string"
};
...
customElements.define('raja-test', WebComponent);
